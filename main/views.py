...
def indexHandler(request):
	...
	open_carts = Cart.objects.filter(
		session_id=request.session.session_key).filter(status=0)
	if open_carts:
		open_cart = open_carts[0]
		cart_items = CartItem.objects.filter(
			cart__id=open_cart.id).filter(status=0)
		cart = open_cart
	...
	if category_id:
		if search_value:
			products = Product.objects.filter(
				category__id=category_id).filter(title__contains=search_value)
		else:
...
def cartHandler(request):
	if request.method == 'POST':
		...
		if action == 'add_to_cart':
			...
			cart_items = CartItem.objects.filter(cart__id=new_cart.id).filter(
				product__id=product_id).filter(status=0)
			if cart_items:
			...
			if new_cart:
				cart_items = CartItem.objects.filter(
					cart__id=new_cart.id).filter(product__id=product_id)
		...
		if action in ['add_to_cart', 'remove_from_cart']:
			...
			if open_carts:
				new_cart = open_carts[0]
				cart_items = CartItem.objects.filter(
					cart__id=new_cart.id).filter(status=0)
